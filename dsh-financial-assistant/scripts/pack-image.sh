...
#
# 产物：dist/dsh-financial-assistant-<版本>-<架构>-offline/（目录）+ 同名 .zip
...
IMAGE="dsh-financial-assistant:$VERSION"
NGINX_IMAGE="nginx:1.27-alpine"
DIST="$ROOT/dist/dsh-financial-assistant-$VERSION-$ARCH-offline"
...
echo "==> [3/5] 导出镜像（dsh-server + nginx 打进一个 tar）"
...
cp "$ROOT/docs/DSH Financial Assistant 服务端部署技术方案.md" "$DIST/技术方案.md" 2>/dev/null || true

cat > "$DIST/DEPLOY.md" <<EOF
# DSH Financial Assistant 离线部署（${VERSION} / ${ARCH}）
...
\`\`\`bash
# 1) 导入镜像（含 dsh-server 与 nginx 两个）
docker load < images.tar.gz
docker images | grep -E "dsh-financial-assistant|nginx"   # 确认在列

# 2) 准备配置
cp .env.example .env                    # 填入 DEEPSEEK_API_KEY、DSH_API_KEYS
cp config/settings.example.yaml config/settings.yaml
#  按需调整模型（默认 deepseek-official + deepseek-chat）与权限预设

# 3) 启动
docker compose up -d
docker compose ps                       # 等 dsh-server 变 healthy

# 4) 验证（内部纯 HTTP，宿主端口 8080；TLS 由上游统一接入层终结）
curl -s http://127.0.0.1:8080/v1/health
SID=\$(curl -s -X POST http://127.0.0.1:8080/v1/sessions \\
  -H "Authorization: Bearer <你的DSH_API_KEY>" -H 'content-type: application/json' -d '{}' \\
  | sed -E 's/.*"sessionId":"([^"]+)".*/\1/')
curl -s -X POST http://127.0.0.1:8080/v1/sessions/\$SID/messages \\
  -H "Authorization: Bearer <你的DSH_API_KEY>" -H 'content-type: application/json' \\
  -d '{"text":"你好"}'
\`\`\`

## 注意
- **重启 dsh-server 必须连带重启 nginx**（共享网络命名空间）：用 \`docker compose up -d\` 或两个都 restart。
...
echo "==> [5/5] 打 zip"
cd "$ROOT/dist"
rm -f "dsh-financial-assistant-$VERSION-$ARCH-offline.zip"
zip -qr "dsh-financial-assistant-$VERSION-$ARCH-offline.zip" "dsh-financial-assistant-$VERSION-$ARCH-offline"

echo ""
echo "完成："
ls -lh "$ROOT/dist/dsh-financial-assistant-$VERSION-$ARCH-offline.zip"
echo "目录：$DIST"
