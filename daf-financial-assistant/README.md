...
```
plugins/daf-server-api/
├── src/
│   ├── index.ts        插件入口：name/inject/apply，装配 hub + routes，读 DAF_API_KEYS
...
├── tsconfig.json
├── cordis.patch.yml    bundle patch：把 daf-server-api 条目 insert 进组合
...
├── package.json        bundles = dsh-base + dsh-web-app + @daf/dsh-server-api；
│                       插件用相对 file: 依赖（构建期拷入）
...
docker-compose.yml      daf-server + nginx（network_mode: service 共享网络命名空间）
.dockerignore
...
docs/
└── DAF Financial Assistant 服务端部署技术方案.md   方案 v2.0 + Phase 0→2 验证记录
```
...
| 位置 | 是什么 |
|---|---|
| `~/.dsh/profiles/dafspike/` | Phase 0/1 本地开发 profile（9610 开发服务），`link:` 软链到插件源码，改代码即生效 |
...
cp .env.example .env                                    # 填 DEEPSEEK_API_KEY 与 DAF_API_KEYS
docker compose up -d --build
curl -s http://127.0.0.1:8080/v1/health
```

> 重启 daf-server 后必须一并重启 nginx（共享网络命名空间，见方案 §7 运维注意）。
...
产物：`dist/daf-financial-assistant-<版本>-<架构>-offline.zip`，内含：

```
images.tar.gz          daf-server + nginx 两个镜像（docker save）
...
```bash
cd plugins/daf-server-api && pnpm install && pnpm build
DAF_API_KEYS=dev-key dsh --profile dafspike --no-open --port 9610
...
鉴权：`Authorization: Bearer <key>`（key 来自 `DAF_API_KEYS`，未配置时受保护路由一律 503）。
